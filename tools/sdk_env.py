...
class RuntimeConfig:
    ...
    def load(cls, args: argparse.Namespace) -> "RuntimeConfig":
        ...
        mirror = args.mirror or os.environ.get("SIFLI_SDK_MIRROR")
        if mirror:
            sources.append({"type": "mirror", "url": str(mirror)})
...
def profile_python_root(config: RuntimeConfig, profile: str) -> str:
    return os.path.join(config.install_root, "python_env", profile)


def current_env_pointer_path(config: RuntimeConfig, profile: str) -> str:
    return os.path.join(profile_python_root(config, profile), "current-env.txt")


def read_current_env_pointer(config: RuntimeConfig, profile: str) -> str:
    pointer_path = current_env_pointer_path(config, profile)
    try:
        with open(pointer_path, "r", encoding="utf-8") as f:
            env_path = f.read().strip()
    except FileNotFoundError as exc:
        raise SDKEnvError(
            f"profile '{profile}' is not installed. Missing environment pointer {pointer_path}. "
            'Run "./install.sh" first (or ".\\install.ps1" on PowerShell).'
        ) from exc
    if not env_path:
        raise SDKEnvError(
            f"profile '{profile}' has an empty environment pointer at {pointer_path}. "
            'Run "./install.sh" first (or ".\\install.ps1" on PowerShell).'
        )
    return os.path.realpath(os.path.expanduser(env_path))


def write_current_env_pointer(config: RuntimeConfig, profile: str, env_path: str) -> None:
    pointer_path = current_env_pointer_path(config, profile)
    atomic_write_text(pointer_path, os.path.realpath(env_path) + "\n")
    log_ok(f"Updated environment pointer {pointer_path}")
...
def download_to_path(url: str, destination: str) -> None:
...
def ensure_cached_artifact(
    ...
    for source in config.sources:
        source_type = source.get("type")
        if source_type == "mirror" and "url" in source:
    ...
    for url in url_candidates:
        ...
        if download_candidate(url, tmp_path, config.offline):
            if matches(tmp_path):
...
def ensure_python_env(config: RuntimeConfig, lock: ProfileLock) -> str:
    ...
    sync_cmd = [
        ...
        "sync",
        "--project",
        os.path.join(repo_root(), lock.python_project_dir),
        "--locked",
        "--python",
        lock.python_version,
        "--no-install-project",
...
def perform_install(
    ...
    write_current_env_pointer(config, lock.profile, env_path)
...
def handle_export(args: argparse.Namespace) -> int:
    ...
    if reasons:
        ...
        if effective_choice == "never":
            ...
            raise SDKEnvError("environment drift detected; run install or allow export to reconcile it")
