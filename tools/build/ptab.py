...
class PtabV3:
    ...
    - Execution address is described by `acc: {region, offset}`.
      `exec_region/exec_offset` are NOT supported in v3.
    ...
    def _normalize_partitions(self, partitions):
        ...
        for p in partitions:
            ...
            if 'exec_region' in p or 'exec_offset' in p:
                raise ValueError(
                    "ptab v3 does not support exec_region/exec_offset. "
                    "Please use acc: {region, offset} instead "
                    f"(partition: {name or '?'})"
                )
