...
Windows 脚本主链路不再要求用户预装系统 Python。当前支持的方式是通过 `uv` 按需准备锁定的 Python 3.12.0 运行时。
...
`export.ps1` 现在直接使用 `install.ps1` 创建的 Python 虚拟环境。如果当前 profile 的虚拟环境尚未安装，或者被手动删除，`export.ps1` 会立即失败，并提示先执行 `.\install.ps1`。
