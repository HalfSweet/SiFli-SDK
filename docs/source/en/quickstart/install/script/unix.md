...
SiFli-SDK no longer relies on the system Python installation. `install.sh` uses `uv` to prepare the locked Python 3.12.0 runtime and dependencies for the active SDK profile.
...
`export.sh` now uses the Python virtual environment created by `install.sh`. If that profile environment has not been installed yet, or was removed manually, `export.sh` will fail immediately and ask you to run `./install.sh` first.
